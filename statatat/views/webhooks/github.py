...
@view_config(name='github', request_method='POST', renderer='string',
...
def github(request):
    ...
    if 'payload' in request.params:
        ...
        topic_extractors = {
            ...
            'repo_owner': lambda i: payload['repository']['owner'
                    ]['email'],
            'author': lambda i: payload['commits'][i]['author']['email'
                    ],
            'committer': lambda i: payload['commits'][i]['committer'
                    ]['email'],
            }
        for (prefix, extractor) in topic_extractors.items():
            for (i, commit) in enumerate(payload['commits']):
                topic = '%s.%s' % (prefix, md5(salt
                                   + extractor(i)).hexdigest())
